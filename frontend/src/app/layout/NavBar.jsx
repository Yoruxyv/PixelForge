...
const ThemeControl = ({ theme, value, onChange, className = '' }) => (
  <label className={`flex items-center gap-2 text-pf-editorial-muted ${className}`}>
    <span className="sr-only">Theme</span>
    {theme === 'dark' ? (
      <svg className="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" aria-hidden="true"><path d="M21 12.8A9 9 0 1 1 11.2 3 7 7 0 0 0 21 12.8Z" /></svg>
    ) : (
      <svg className="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" aria-hidden="true"><circle cx="12" cy="12" r="4" /><path d="M12 2v2M12 20v2M4.93 4.93l1.42 1.42M17.66 17.66l1.41 1.41M2 12h2M20 12h2M4.93 19.07l1.42-1.42M17.66 6.34l1.41-1.41" /></svg>
    )}
    <select value={value} onChange={(event) => onChange(event.target.value)} className="cursor-pointer bg-transparent text-xs font-bold uppercase tracking-[0.1em] text-pf-editorial-muted outline-none transition-colors hover:text-pf-editorial-ink" aria-label="Theme">
    ...
    <nav className="sticky top-0 z-50 w-full border-b border-pf-editorial-line bg-pf-editorial-base/95 backdrop-blur-lg" aria-label="Primary navigation">
      <div className="mx-auto flex h-16 max-w-pf-workspace items-center justify-between px-pf-gutter">
        ...
        <div className="hidden items-center gap-2 lg:flex">
          ...
          <a href="https://github.com/Yoruxyv/PixelForge" target="_blank" rel="noopener noreferrer" className="text-sm font-semibold text-pf-editorial-muted transition-colors hover:text-pf-editorial-ink">GitHub</a>
        </div>
      ...
      {isMobileMenuOpen && (
        <div id="mobile-navigation" className="max-h-[calc(100vh-4rem)] overflow-y-auto border-t border-pf-editorial-line bg-pf-editorial-base px-pf-gutter py-5 lg:hidden">
          ...
          <ThemeControl theme={theme} value={themePreference} onChange={onThemeChange} className="mt-5 border-t border-pf-editorial-line px-3 pt-5" />
        </div>
      )}
    </nav>
  );
